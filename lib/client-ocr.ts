...
export async function performClientOCR(file: File, onProgress?: (progress: number) => void): Promise<OCRResult> {
  const startTime = Date.now()

  console.log("[v0] Starting client-side OCR for:", file.name)

  const worker = await createWorker("eng", 1, {
    logger: (m) => {
      console.log(`[v0] OCR ${m.status}: ${Math.round((m.progress || 0) * 100)}%`)
  ...
  try {
    ...
    await worker.setParameters({
      tesseract_pageseg_mode: "1", // Automatic page segmentation with OSD
      tesseract_ocr_engine_mode: "2", // Use LSTM OCR engine
    })

    console.log("[v0] Starting text recognition")
    const {
      data: { text, confidence },
    } = await worker.recognize(file)

    const processingTime = Date.now() - startTime
    console.log(`[v0] OCR completed in ${processingTime}ms with confidence: ${confidence}%`)
