...
import { performClientOCR, validateImageFile, type OCRResult } from "@/lib/client-ocr"
...
export function FileUpload() {
  ...
  const processBase64ImageOCR = async (
    ...
    console.log("[v0] Processing base64 image with OCR:", fileName)

    // Convert base64 to blob for Tesseract
    const response = await fetch(base64Data)
    const blob = await response.blob()

    return await performClientOCR(new File([blob], fileName, { type: "image/png" }), onProgress)
  }
    ...
    for (const uploadedFile of uploadedFiles.filter((f) => f.status === "pending")) {
      ...
      try {
        if (uploadedFile.file.type.startsWith("image/")) {
          console.log("[v0] Processing image with client-side OCR:", uploadedFile.file.name)
          ...
          const result: OCRResult = await performClientOCR(uploadedFile.file, (progress) => {
            setUploadedFiles((prev) =>
              prev.map((f) => (f.id === uploadedFile.id ? { ...f, progress: Math.round(progress * 100) } : f)),
            )
          })
          ...
          console.log("[v0] Processing PDF with server-side extraction:", uploadedFile.file.name)

          const formData = new FormData()
          formData.append("file", uploadedFile.file)

          console.log("[v0] FormData created with file:", uploadedFile.file.name, "size:", uploadedFile.file.size)
          ...
          if (!response.ok) {
            ...
            if (contentType && contentType.includes("application/json")) {
              ...
              try {
                ...
                console.error("[v0] Failed to parse error text:", textError)
                errorMessage = `Server error (${response.status}): ${response.statusText}`
              }
            }
          ...
          if (result.requiresClientOCR && result.isScannedPDF && result.images) {
            console.log("[v0] Processing scanned PDF with", result.images.length, "pages")
            ...
            for (let i = 0; i < result.images.length; i++) {
              ...
              console.log(`[v0] Processing page ${image.pageNumber} with OCR`)
              ...
              const ocrResult = await processBase64ImageOCR(
                ...
                (progress) => {
                  const overallProgress = ((i + progress) / result.images.length) * 100
                  setUploadedFiles((prev) =>
                    prev.map((f) =>
                      f.id === uploadedFile.id
                        ? {
            ...
            // Regular PDF text extraction result
            setUploadedFiles((prev) =>
              prev.map((f) =>
                f.id === uploadedFile.id
                  ? {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-8">
          ...
          >
            ...
            <p className="text-muted-foreground mb-4">Drag and drop PDF files or images, or click to browse</p>
            <p className="text-sm text-muted-foreground">Supported formats: PDF, PNG, JPG, JPEG, GIF, BMP, TIFF</p>
          </div>
        </CardContent>
      </Card>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {uploadedFile.status === "completed" && uploadedFile.confidence && (
                        <span className="ml-2">• {Math.round(uploadedFile.confidence)}% confidence</span>
                      )}
                      {uploadedFile.status === "completed" && uploadedFile.processingTime && (
                        <span className="ml-2">• {(uploadedFile.processingTime / 1000).toFixed(1)}s</span>
                      )}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {uploadedFile.status === "pending" && <span className="text-sm text-muted-foreground">Ready</span>}
                    ...
                    {uploadedFile.status === "completed" && <span className="text-sm text-green-600">Completed</span>}
      ...
      {errorFiles.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-destructive">Processing Errors</h3>
          {errorFiles.map((file) => (
            <ProcessingStatus key={file.id} fileName={file.file.name} status="error" error={file.error} />
          ))}
        </div>
      )}
